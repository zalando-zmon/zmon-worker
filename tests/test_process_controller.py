...
def test_process_plus_ping_aggregations(monkeypatch):
    ...
    aggregated_pings_no_data = {'tasks_per_sec': -1, 'tasks_per_min': -1, 'percent_idle': -1, 'interval': 0,
                                'tasks_done': -1, 'pings_received': -1, 'time_per_task': -1}
    ...
    aggregated_results = {
        ...
        'time_per_task': sum([p['task_time'] for p in pings_sample]) / sum_tasks,
    }
    ...
    aggregated_results_last_ping = {
        ...
        'pings_received': 1,
        'time_per_task': pings_sample[-1]['task_time'] / pings_sample[-1]['tasks_done'],
    }
